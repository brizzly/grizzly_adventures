...
bool HasJoypad()
{
	bool is_joypad = false;// (bool)[KJoypad Get_HasJoypad];
	return is_joypad;
}
...
#ifdef WIN_GRIZZ
	fenetre = KPTK::createKWindow(K_DIRECTX9, ScreenX, ScreenY);
	//fenetre->toggleFullScreen(false);
#else
	fenetre = KPTK::createKWindow(K_MAC, ScreenX, ScreenY);
#endif

	res = fenetre->createGameWindow(ScreenX,ScreenY,32,windowed, (char*)NAME);
	fenetre->setMaxFrameRate( 60 );
	fenetre->setClearColor(0, 0, 0, 1);

	InitializeInput(fenetre);
	if (!windowed) {
		KInput::hidePointer();
	}
...
{
	char * completePath = szBitmap;
#ifdef WIN_GRIZZ
	char fullPath[1024];
	sprintf(fullPath, "datas/gfx/%s.png", szBitmap);
	completePath = fullPath;
#endif

	char * finalPath = KMiscTools::makeFilePath(completePath);
	...
	if(kg->loadPicture(finalPath, true, true ) == false)
	{
		delete kg;
		kg = NULL;
	}
