...
{
	char * path = KMiscTools::makeTempFilePath(filename);
...
{
	const char * path = KMiscTools::makeTempFilePath(filename);
...
void game_ResetConfig()
{
	...
	char * p = KMiscTools::makeTempFilePath(CONFIGFILEPATH);
	printf("path = %s\n", p);
	WriteToFile(CONFIGFILEPATH, (char*)"0");
	
	int fh = open(p, O_WRONLY);
	//WriteToFile(fh, (char*)"0");
	if(fh == -1) {
		return;
	}
    
 	// header
	fileWrite(fh, CONFIGHEADER);
	fileWrite(fh, "\n");
...
void g_Codage(char *nom_fichier, short mode)
{
	...
	int fh = open(KMiscTools::makeTempFilePath( nom_fichier ), O_RDONLY);
	if(fh == -1) {
		return;
	}
	...
	fh = open(KMiscTools::makeTempFilePath(nom_fichier), O_WRONLY);
	if(fh == -1) {
		return;
	}
	write(fh, datas, i);
...
void g_LoadConfig()
{
	...
	int fh = open(KMiscTools::makeTempFilePath(CONFIGFILEPATH), O_RDONLY);
	if(fh == -1)
    {
	...
	{
		while((gotten = read(fh,buffer,chunksize)))
		{
			totalsize += chunksize;
			buffer[gotten] = '\0';
	...
	fh = open(KMiscTools::makeTempFilePath(CONFIGFILEPATH), O_RDONLY);
	if(fh == -1) {
		return;
	}
...
void g_SaveConfig()
{
    int fh = open(KMiscTools::makeTempFilePath(CONFIGFILEPATH), O_WRONLY);
	if(fh == -1) {
		return;
	}
	fileWrite(fh, CONFIGHEADER);
...
void g_LoadSounds(char *FolderPath)
{
	...
	FolderPath = "";
	sprintf(path, "%s%s", FolderPath, "boing2.wav.caf");			lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 0
	sprintf(path, "%s%s", FolderPath, "fruit.wav.caf");				lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 1
	sprintf(path, "%s%s", FolderPath, "bee1.wav.caf");				lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 2
	sprintf(path, "%s%s", FolderPath, "breakjarre.wav.caf");		lb_LoadSound(KMiscTools::makeFilePath(path), 1, false);	// 3
	sprintf(path, "%s%s", FolderPath, "rebon.wav.caf");				lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 4
	sprintf(path, "%s%s", FolderPath, "menu.wav.caf");				lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 5
	sprintf(path, "%s%s", FolderPath, "click.wav.caf");				lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 6
	sprintf(path, "%s%s", FolderPath, "jump1.wav.caf");				lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 7
	sprintf(path, "%s%s", FolderPath, "lost.wav.caf");				lb_LoadSound(KMiscTools::makeFilePath(path), 1, false);	// 8
	sprintf(path, "%s%s", FolderPath, "lifeup.wav.caf");			lb_LoadSound(KMiscTools::makeFilePath(path), 1, false);	// 9
	sprintf(path, "%s%s", FolderPath, "grizzly_abeille.wav.caf");	lb_LoadSound(KMiscTools::makeFilePath(path), 3, false);	// 10
	sprintf(path, "%s%s", FolderPath, "grizzly_chenille.wav.caf");	lb_LoadSound(KMiscTools::makeFilePath(path), 3, false);	// 11
	sprintf(path, "%s%s", FolderPath, "grizzly_clef.wav.caf");		lb_LoadSound(KMiscTools::makeFilePath(path), 3, false);	// 12
	sprintf(path, "%s%s", FolderPath, "grizzly_cochon.wav.caf");	lb_LoadSound(KMiscTools::makeFilePath(path), 3, false);	// 13
	sprintf(path, "%s%s", FolderPath, "grizzly_fantome.wav.caf");	lb_LoadSound(KMiscTools::makeFilePath(path), 3, false);	// 14
	sprintf(path, "%s%s", FolderPath, "lifeup.wav.caf");			lb_LoadSound(KMiscTools::makeFilePath(path), 3, false);	// 15
	sprintf(path, "%s%s", FolderPath, "banana.wav.caf");			lb_LoadSound(KMiscTools::makeFilePath(path), 3, false);	// 16
	sprintf(path, "%s%s", FolderPath, "fruit2.wav.caf");			lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 17
	sprintf(path, "%s%s", FolderPath, "fruit3.wav.caf");			lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 18
	sprintf(path, "%s%s", FolderPath, "explosion.wav.caf");			lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 19
	sprintf(path, "%s%s", FolderPath, "rebon2.wav.caf");			lb_LoadSound(KMiscTools::makeFilePath(path), 2, false);	// 20
	sprintf(path, "%s%s", FolderPath, "breakjarre2.wav.caf");		lb_LoadSound(KMiscTools::makeFilePath(path), 1, false);	// 21
}
...
void g_InitGame()
{
    ...
    int fh = open(level_name, O_RDONLY);
	if(fh == -1) {
		lb_PlayMP3(KMiscTools::makeFilePath("menu.mp3"), gMusicVol, true);
		g_InitPresentation();
		return;
	}
    
	read(fh, gBlocDefinition, sizeof(int)*MAX_TILES);
	close(fh);
...
void g_PlayMusic()
{
	...
	{
		case 1:
		lb_PlayMP3(KMiscTools::makeFilePath("world1.mp3"), gMusicVol, true);
		break;

		case 2:
		lb_PlayMP3(KMiscTools::makeFilePath("world2.mp3"), gMusicVol, true);
		break;

		case 3:
		lb_PlayMP3(KMiscTools::makeFilePath("world3.mp3"), gMusicVol, true);
		break;
	}
}
...
bool g_ReInitGame()
{
	...
	char episodeStr[32];
	char stageStr[32];
	sprintf(episodeStr, "Episode_%d", gEpisodeID);
	sprintf(stageStr, "Stage_%d", level_id);
	//gameanalytics::GameAnalytics::addProgressionEvent(gameanalytics::EGAProgressionStatus::Start, episodeStr, stageStr, NULL/*level_name*/);
...
void g_InitLoading()
{
	lb_PlayMP3(KMiscTools::makeFilePath("intro.mp3"), gMusicVol, false);
...
void g_EndLevelHit()
{
	...
	lb_PlayMP3(KMiscTools::makeFilePath("level_finish.mp3"), gMusicVol, false);
...
void g_InitGameover()
{
	lb_PlayMP3(KMiscTools::makeFilePath("gameover.mp3"), gMusicVol, false);
...
//			g_DispSharewareWarning();
		...
		case PS_CONGRAT:
		{
			...
			{
				g_UnInitGame();
				//lb_PlayMP3(KMiscTools::MakeFilePath("menu.mp3"), gMusicVol);
				g_InitPresentation();
				return;
			}
...
//	if(table)			DeleteSurface(table);
	...
	sprintf(path, "world%d.png", world_id);
//	if(!background)
//	{
...
//		DDReLoadBitmap(background, path);
	...
	sprintf(path, "paralaxs%d.png", world_id);
//	if(!paralax1)
//	{
